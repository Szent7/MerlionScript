...
func createNewPositionsMS(ctx context.Context) error {
	...
	default:
		...
		catMeta, err := sklad.GetCatMeta(keeper.K.GetSkladCat())
		...
		for _, item := range *items {
			...
			default:
				...
				if len(msItems.Rows) == 0 {
					if createNewItemMS(&item, copyItem, catMeta, &counter, dbInstance) {
						createdItems++
					}
...
func createNewItemMS(item *typesDB.Codes, copyItem *typesDB.Codes, Catalog restTypes.Meta, counter *int64, dbInstance *db.DBInstance) bool {
	...
	var newItem restTypes.CreateItem
	if Catalog.Href == "" {
